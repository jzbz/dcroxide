...
- `dcroxide-blockchain` — the beginnings of the chain engine: dcrd's
  ...
  machinery, the agenda threshold state machine, the agenda-driven algorithm selectors, the chain persistence formats, and the context-free transaction validation layer, pinned by dcrd's own test vectors and by synthetic-chain
