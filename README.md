...
- `dcroxide-rpc` — RPC server components from dcrd's
  ...
  stake-query, mempool/connection, tx/utxo lookup, peer/address, and
  submission/control commands) over a Server scaffold with the
  chain, mempool, sync and connection managers, indexes, database,
  filterer, log manager, fee estimator, and clock behind trait
