...
const SBSS_VIOLATIONS: &[(i64, Hash)] = &[
    (
        1106817,
        Hash([
            0x3b, 0x61, 0x39, 0xb9, 0xe3, 0xe5, 0x2d, 0x88, 0x09, 0xdb, 0x3a, 0x2a, 0x88, 0x39,
            0x66, 0x78, 0x5e, 0x5b, 0xfd, 0x31, 0x43, 0xd5, 0xe0, 0x6e, 0xaa, 0xc8, 0x12, 0x5f,
            0xdc, 0xec, 0x75, 0x08,
        ]),
    ),
    (
        1106831,
        Hash([
            0x27, 0xee, 0x80, 0x7c, 0xc3, 0xb0, 0xae, 0xfe, 0xcd, 0xba, 0x38, 0x7b, 0xa6, 0x0c,
            0x44, 0xa3, 0x3e, 0xd4, 0x53, 0x08, 0x90, 0xd2, 0x0f, 0x48, 0x33, 0xcd, 0x37, 0x89,
            0x35, 0x71, 0x4e, 0x71,
        ]),
    ),
    (
        1106837,
        Hash([
            0x18, 0xcd, 0xda, 0x2c, 0x3c, 0xa3, 0x38, 0x9f, 0xd6, 0x88, 0x3f, 0x26, 0x3c, 0x64,
            0xa7, 0xcd, 0x37, 0xbd, 0x40, 0x7f, 0xf2, 0x68, 0x3e, 0xc1, 0xe3, 0x34, 0xa3, 0xef,
            0xa4, 0x09, 0xba, 0xae,
        ]),
    ),
    (
        1106854,
        Hash([
            0x39, 0xa4, 0xc7, 0x70, 0x14, 0x15, 0xc8, 0x28, 0xcf, 0xb3, 0xdb, 0xd2, 0xa9, 0xc2,
            0x55, 0xd7, 0xa6, 0xac, 0x19, 0x93, 0x47, 0x2e, 0xaa, 0x84, 0x70, 0xdd, 0xd6, 0xd9,
            0x66, 0x82, 0x22, 0x17,
        ]),
    ),
    (
        1106860,
        Hash([
            0x6d, 0x4e, 0xfc, 0x70, 0x92, 0x9b, 0x1f, 0x06, 0x58, 0xfd, 0x7d, 0x45, 0x66, 0x57,
            0x03, 0x5e, 0xc2, 0x52, 0xf4, 0xa1, 0xdc, 0x74, 0x94, 0x96, 0x03, 0x9f, 0x60, 0x1b,
            0xed, 0xd3, 0xd6, 0x20,
        ]),
    ),
    (
        1107195,
        Hash([
            0xd3, 0xb2, 0x0a, 0xac, 0x85, 0xab, 0xd3, 0x4d, 0xa6, 0xde, 0xe8, 0x6b, 0x82, 0xf1,
            0x72, 0x97, 0x8d, 0xcf, 0xb3, 0x6f, 0x90, 0xb1, 0x95, 0xb7, 0x1c, 0x63, 0x0c, 0xfe,
            0x98, 0x18, 0x2e, 0x5d,
        ]),
    ),
];
...
const SBSS_VIOLATIONS_TESTNET3: &[(i64, Hash)] = &[(
    1980161,
    Hash([
        0x58, 0xbb, 0xdc, 0xb3, 0x76, 0xa6, 0xb7, 0xc6, 0x5f, 0xb7, 0xad, 0x63, 0xd8, 0xb7, 0xbe,
        0x4c, 0x44, 0x9e, 0x0a, 0xe1, 0xdb, 0xc1, 0xb5, 0xbb, 0xc5, 0x7b, 0x77, 0x1a, 0xed, 0x6e,
        0x1b, 0x45,
    ]),
)];

/// One network's same-block-stake-spend exceptions, as height and the
/// hash of the transaction whose output the stake transaction spends.
pub type SbssExceptions = &'static [(i64, Hash)];

/// The same-block-stake-spend exceptions in dump order, exposed so a test
/// can pin the exact bytes against dcrd's literals the way
/// [`dcp0005_constants`] does for DCP0005.
///
/// These are the one part of the check that no other test can reach.  The
/// rejection arm is pinned by dcrd's own full block battery, but nothing
/// in that battery spends a grandfathered utxo, so a transcription error
/// here — a reversed hash, a mistyped height — would pass every test in
/// the repository and surface only as an initial mainnet sync halting at
/// height 1106817.  Hence pinning the bytes directly.
pub fn sbss_violation_constants() -> (SbssExceptions, SbssExceptions) {
    (SBSS_VIOLATIONS, SBSS_VIOLATIONS_TESTNET3)
}

/// Whether the given network, height and utxo hash name one of the
/// grandfathered same-block stake spends, exposed for the same reason as
/// [`sbss_violation_constants`].
pub fn is_sbss_violation_for(params: &Params, tx_height: i64, utxo_hash: &Hash) -> bool {
    is_sbss_spend_violation(params, tx_height, utxo_hash)
}
    ...
    for (idx, (tx, tx_hash)) in txs.iter().zip(tx_hashes).enumerate() {
        ...
        if !stake_tree {
            view.connect_regular_transaction(
                ...
                vote_bits_approve_parent(node_vote_bits),
                stxos.as_deref_mut(),
                is_treasury_enabled,
            )?;
        } else {
