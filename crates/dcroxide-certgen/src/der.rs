...
/// Whether a string fits ASN.1 PrintableString the way Go's marshaller
/// decides it (`encoding/asn1` `makeBody` calls `isPrintable` with
/// `rejectAsterisk` and `rejectAmpersand`).
///
/// Go's parser accepts `*` and `&` in a PrintableString, because
/// certificates in the wild carry them, but its marshaller does not write
/// them there: a wildcard host or `Smith & Co` becomes a UTF8String.
...
#[cfg(test)]
mod tests {
    use super::directory_string;

    /// Go marshals a pkix.Name value as a PrintableString (tag 0x13) only
    /// when every character is in the X.680 set, and never with `*` or `&`,
    /// which its parser tolerates but its marshaller writes as UTF8String
    /// (tag 0x0c).
    #[test]
    fn directory_string_tags_as_go_marshals() {
        for (value, tag) in [
            ("dcrd autogenerated cert", 0x13),
            ("host-1.example", 0x13),
            ("a'b(c)d+e,f/g:h=i?j", 0x13),
            ("*.example.com", 0x0c),
            ("Smith & Co", 0x0c),
            ("under_score", 0x0c),
            ("héllo", 0x0c),
            ...
            let der = directory_string(value);
            assert_eq!(der[0], tag, "{value:?}");
            assert_eq!(&der[2..], value.as_bytes(), "{value:?}");
        }
    }
}
