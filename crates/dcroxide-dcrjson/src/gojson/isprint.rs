...
//! Go's `strconv.IsPrint` (`strconv/quote.go`) over the tables it
//! carries (`strconv/isprint.go`, generated by `makeisprint.go`), copied
//! from Go 1.26.5.  dcrd's release image builds with Go 1.26.2 at the
//! parity pin (`contrib/docker/Dockerfile`); the tables follow the
//! Unicode version, which only a minor Go release moves.
//!
//! `IsPrint` is `unicode.IsPrint` -- letters, marks, numbers,
//! punctuation, symbols and the ASCII space -- answered from these
//! compact tables so `strconv` need not link the Unicode package.  The
//! Rust standard library exposes no general category, so the port
//! carries the same tables to decide exactly the runes `strconv.Quote`
//! (the `%q` verb) escapes.  A Go release that moves to a newer Unicode
//! version regenerates them; re-copy them when the parity toolchain
//! moves.

/// Go `strconv.IsPrint`.
pub(super) fn is_print(r: char) -> bool {
    let r = u32::from(r);
