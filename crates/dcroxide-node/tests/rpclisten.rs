...
/// Start a TLS listener over a certificate pair generated on `curve`, the
/// way the daemon's first start generates one, handing back the PEM
/// certificate and key it serves.
fn serve_tls(
    ...
    (dir, listener, port, cert_pem, key_pem)
}
...
fn serves_tls_with_a_generated_certificate() {
    ...
    let (_dir, listener, port, cert_pem, _key_pem) = serve_tls(dcroxide_certgen::Curve::P256);
...
/// dcrd serves a P-521 certificate over TLS 1.2 and 1.3. In TLS 1.3 Go
/// signs each handshake with `ECDSAWithP521AndSHA512`
/// (`crypto/tls/auth.go:227`); in TLS 1.2 it signs with the first ECDSA
/// hash the client lists (`:211-219`, `:290-296`). The port signs with
/// `ECDSAWithP521AndSHA512` in both. A client offering only that scheme
/// completes both versions and verifies every handshake signature against
/// the key. A client that does not offer it gets no handshake; in TLS 1.3
/// that is also where Go's server stops, finding nothing in common
/// (`crypto/tls/auth.go:297`), while in TLS 1.2 Go would serve it, as
/// PARITY records.
#[test]
fn serves_tls_with_a_p521_certificate() {
    use std::io::{Read, Write};
    ...
    /// Pins the certificate and verifies each handshake signature with the
    /// P-521 key, counting the signatures it verified.
    struct P521Pinned {
        cert: Vec<u8>,
        key: p521::ecdsa::VerifyingKey,
        verified: AtomicUsize,
    }
    impl std::fmt::Debug for P521Pinned {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            f.write_str("P521Pinned")
        }
    }
    impl P521Pinned {
        fn check(
            ...
            if cert.as_ref() != self.cert.as_slice()
                || dss.scheme != rustls::SignatureScheme::ECDSA_NISTP521_SHA512
            {
                return Err(bad());
            }
            let signature = p521::ecdsa::Signature::from_der(dss.signature()).map_err(|_| bad())?;
            self.key.verify(message, &signature).map_err(|_| bad())?;
            self.verified.fetch_add(1, Ordering::SeqCst);
            Ok(HandshakeSignatureValid::assertion())
        }
    }
    impl ServerCertVerifier for P521Pinned {
        fn verify_server_cert(
            &self,
        ...
        fn supported_verify_schemes(&self) -> Vec<rustls::SignatureScheme> {
            vec![rustls::SignatureScheme::ECDSA_NISTP521_SHA512]
        }
    }

    /// Offers the schemes ring verifies, none of which is 0x0603.
    #[derive(Debug)]
    struct RingSchemesOnly(Vec<rustls::SignatureScheme>);
    impl ServerCertVerifier for RingSchemesOnly {
        fn verify_server_cert(
            &self,
        ...
        fn verify_tls12_signature(
            ...
            Err(rustls::Error::General(
                "no signature should reach this client".into(),
            ))
        }
        fn verify_tls13_signature(
            &self,
    ...
    let (_dir, listener, port, cert_pem, key_pem) = serve_tls(dcroxide_certgen::Curve::P521);
    let pinned = CertificateDer::pem_slice_iter(&cert_pem)
        .next()
        .expect("one cert")
        .expect("parse cert")
        .as_ref()
        .to_vec();
    let key_der = PrivateKeyDer::from_pem_slice(&key_pem).expect("parse key");
    let verifying_key = match dcroxide_certgen::gentool::parse_private_key(
        "EC PRIVATE KEY",
        key_der.secret_der(),
    ) {
        Ok(dcroxide_certgen::gentool::ToolKeyPair::P521(key)) => *key.verifying_key(),
        _ => panic!("the generated key is P-521"),
    };
    ...
    for version in [&rustls::version::TLS13, &rustls::version::TLS12] {
        let verifier = Arc::new(P521Pinned {
            cert: pinned.clone(),
            key: verifying_key,
        ...
        let session =
            rustls::ClientConnection::new(config, ServerName::try_from("localhost").expect("name"))
                .expect("client");
        let tcp = TcpStream::connect(("127.0.0.1", port)).expect("connect");
        ...
        assert!(response.contains("\"result\":0"), "{version:?}: {response}");
        assert_eq!(stream.conn.protocol_version(), Some(version.version));
        assert_eq!(
            verifier.verified.load(Ordering::SeqCst),
            1,
            "{version:?}: one handshake signature, verified with the P-521 key"
        );
    }
    ...
    let ring_schemes = rustls::crypto::ring::default_provider()
        .signature_verification_algorithms
        .supported_schemes();
    assert!(!ring_schemes.contains(&rustls::SignatureScheme::ECDSA_NISTP521_SHA512));
    for version in [&rustls::version::TLS13, &rustls::version::TLS12] {
        let config = Arc::new(
            rustls::ClientConfig::builder_with_protocol_versions(&[version])
                .dangerous()
                .with_custom_certificate_verifier(Arc::new(RingSchemesOnly(ring_schemes.clone())))
                .with_no_client_auth(),
        );
        let session =
            rustls::ClientConnection::new(config, ServerName::try_from("localhost").expect("name"))
        ...
        let tcp = TcpStream::connect(("127.0.0.1", port)).expect("connect");
        let mut stream = rustls::StreamOwned::new(session, tcp);
        let outcome = stream.write_all(request.as_bytes()).and_then(|()| {
            let mut response = String::new();
            stream.read_to_string(&mut response).map(|_| response)
        });
        let err = outcome.expect_err("a client without ecdsa_secp521r1_sha512 must not be served");
        // TLS 1.3 refuses with a `handshake_failure` alert, as Go does. TLS
        // 1.2 fails at the key's scheme choice with a bare
        // `Error::General("incompatible signing key")` and no alert
        // (`rustls/src/server/tls12.rs:384-385`), so the client sees the
