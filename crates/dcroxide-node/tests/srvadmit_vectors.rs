...
//! in-package dump driving dcrd's real handleAddPeer, DonePeer, and
//! isWhitelisted on live piped serverPeers at release-v2.1.5, reading
//! the peer state maps and banned hosts directly — the ban gate with
//! expiry, outbound placement, and the untracked-peer connection time
//! update — compared row for row, where the pin kept the behaviour.
//!
//! The dump's rows for dcrd's `disconnectPeer`, which the pin no
//! longer has, were dropped along with the port of it.
...
use dcroxide_node::server::{
    ...
    PeerState, handle_add_peer, is_whitelisted, wire_to_addrmgr_net_address,
};
...
fn server_peer_admission_matches_dcrd() {
    ...
    // DonePeer removes the tracked outbound peer and returns.
