...
- [ADR-0008](0008-clippy-lint-policy.md) — Curated lint set: `iter_over_hash_type`/`allow_attributes`/`unreachable_pub` adopted, cast lints refused or deferred, with the measured fallout *(Accepted; addendum 2026-09-25 — `dcroxide-winsvc` is the one audited exception to the no-unsafe rule; addendum 2026-09-26 — the crate-level `arithmetic_side_effects` allows are gone)*
