...
| `rustls` | 0.23.43 | RPC/websocket TLS | accepted | Same reachability as `ring`. |
| `getrandom` | 0.2.17 / 0.3.4 / 0.4.3 | entropy | accepted | Three majors coexist. 0.3.4 is what dcroxide calls directly (nonces, the orphan-eviction draw, secret files) and what `rand_core` 0.9.5 uses. 0.2.17 arrives from three directions: through `k256`'s `rand_core` 0.6.4 in `dcroxide-dcrec`, which puts it in every build that verifies a signature; through `p256`/`p384`/`p521`'s in `dcroxide-certgen`; and through `ring`, which is `rustls`' crypto provider for both the RPC server and `ureq` — so it is on the production TLS path from two directions, not one. 0.4.3 is `tempfile`'s and reaches the build only through dev-dependencies, so it is absent from the shipped binary. Worth collapsing the two production majors when the certgen curve crates and `ring` allow. |
| `p256`, `p384`, `p521` | 0.13.2, 0.13.1, 0.13.3 | RPC certificate generation | accepted | Pure Rust and themselves `#![forbid(unsafe_code)]`, which is why they sit here rather than under a C provider. `dcroxide-certgen` uses them to generate and self-sign the RPC certificate for `--tlscurve`; the field arithmetic is fiat-crypto output, but all three ship the same warning that the curve arithmetic "has never been independently audited" and that constant-time generated assembly has not been assessed. `p521` additionally draws its ECDSA nonce straight from the RNG (`src/ecdsa.rs:145`, `TODO: use RFC6979` still open at `:40`) where `ring` folds the message in to hedge — which is the reason `--tlscurve=P-521` is refused for TLS *serving* rather than merely unimplemented; see PARITY. Signing one certificate is a single operation over a message the port chose, so the exposure is not the handshake path's. |
...
`redb` ~27k. ADR-0002's precedent applies to code small enough to own,
